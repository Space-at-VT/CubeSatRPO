...
%             GA_STM@FormationFlying(initStruct.params{5},initStruct.params{6},initStruct.params{2});
            ...
            GA_STM.J2 = initStruct.params{3};
            GA_STM.tol = initStruct.params{4};
            GA_STM.t0 = initStruct.params{5};
            GA_STM.numPeriod = initStruct.params{6};
            GA_STM.safetyAltitude = initStruct.params{7};
            GA_STM.chiefOrbitDescription = initStruct.initChiefDescription;
            GA_STM.deputyOrbitDescription = initStruct.initDeputyDescription;
            
            method = GA_STM.chiefOrbitDescription;
            switch method
                case 'Classical'
                    GA_STM.kepElemsInit = initStruct.Elements;
                    GA_STM.ChiefElemsNSMean = COE_to_Nonsingular(GA_STM.kepElemsInit,GA_STM.tol);
                    GA_STM.ChiefElemsNSMean = GA_STM.ChiefElemsNSMean(:);
                    [~,GA_STM.ChiefOsc] = MeanToOsculatingElements(GA_STM.J2,GA_STM.ChiefElemsNSMean,GA_STM.Req,GA_STM.mu);
                case 'Nonsingular'
                    GA_STM.ChiefElemsNSMean = initStruct.Elements;
                    GA_STM.kepElemsInit = Nonsingular_to_COE(GA_STM.ChiefElemsNSMean);
                    [~,GA_STM.ChiefOsc] = MeanToOsculatingElements(GA_STM.J2,GA_STM.ChiefElemsNSMean,GA_STM.Req,GA_STM.mu);
            end
            
            method = GA_STM.deputyOrbitDescription;
            switch method
                case 'Cartesian'
                    GA_STM.initialConditions = initStruct.RelInitState;
                case 'Relative Nonsingular'
                    GA_STM.DepElemsInitNS = GA_STM.ChiefElemsNSMean + initStruct.RelInitState;
                    [~,GA_STM.DepOsc] = MeanToOsculatingElements(GA_STM.J2,GA_STM.DepElemsInitNS,GA_STM.Req,GA_STM.mu);
                    deltaElems = GA_STM.DepOsc - GA_STM.ChiefOsc;
                    GA_STM.initialConditions = SigmaMatrix(GA_STM.J2,GA_STM.ChiefOsc,GA_STM.Req,GA_STM.mu)*deltaElems;
                case 'Relative Classical'
                    GA_STM.DepElemsInit = GA_STM.kepElemsInit + initStruct.RelInitState;
                    GA_STM.DepElemsInitNS =  COE_to_Nonsingular(GA_STM.DepElemsInit,GA_STM.tol);
                    [~,GA_STM.DepOsc] = MeanToOsculatingElements(GA_STM.J2,GA_STM.DepElemsInitNS,GA_STM.Req,GA_STM.mu);
                    deltaElems = GA_STM.DepOsc - GA_STM.ChiefOsc;
                    GA_STM.initialConditions = SigmaMatrix(GA_STM.J2,GA_STM.ChiefOsc,GA_STM.Req,GA_STM.mu)*deltaElems;
            end
        ...
        function GA_STM = makeTimeVector(GA_STM)
            method = GA_STM.chiefOrbitDescription;
            switch method
                case 'Classical'
                    n = sqrt(GA_STM.mu/GA_STM.kepElemsInit(1)^3);
                    if isempty(GA_STM.t0)
                        GA_STM.t0 = 0;
                    else
                    end
                    GA_STM.period = 2*pi/n;
                    tf = GA_STM.numPeriod*GA_STM.period;
                    GA_STM.time = linspace(GA_STM.t0,tf,100.*GA_STM.numPeriod);
                case 'Nonsingular'
                    n = sqrt(GA_STM.mu/GA_STM.ChiefElemsNSMean(1)^3);
                    if isempty(GA_STM.t0)
                        GA_STM.t0 = 0;
                    else
                    end
        ...
        function GA_STM = PropagateGASTM(GA_STM)
            GA_STM.Phi = PHI_GA_STM(GA_STM.time,GA_STM.J2,GA_STM.ChiefOsc,GA_STM.ChiefElemsNSMean,GA_STM.Req,GA_STM.mu,GA_STM.tol);
            for ii = 1:length(GA_STM.time)
                GA_STM.X(:,ii) = GA_STM.Phi(:,:,ii)*GA_STM.initialConditions;
            end
        end
        
        function GA_STM = plotGAOrbit(GA_STM)
            x = 1e-3.*GA_STM.X(1,:);
            y = 1e-3.*GA_STM.X(3,:);
            z = 1e-3.*GA_STM.X(5,:);
