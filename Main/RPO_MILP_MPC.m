...
% Kalman filter setup
kalman = kalmanSetup([x0;y0;z0], [vx0;vy0;vz0]);
...
while  mFuel > 0  %computation will end when fuel mass is 0
    ...
    if isempty(fval) %Deputy leaves bounds
        ...
        accel = [ux(iter)/m+uhx(iter);
                 uy(iter)/m+uhy(iter);
                 uz(iter)/m+uhz(iter)];
        
        % Run Kalman Filter
        kalman = stateEst([x(iter+1);y(iter+1);z(iter+1)],...
                          [vx(iter+1);vy(iter+1);vz(iter+1)],...
                           accel,...
                           kalman,...
                           t0);
