...
%         
        % Update fuel mass
        mFuel = mFuel-mdot*dt*sum(abs([uxT,uyT,uzT])); %calculate fuel loss based on number of times controls were turned on (mult. by dt=1s)
        m = mDry+mFuel;        
        ...
        simstr = datestr(T/3600/24, 'DD HH:MM:SS');
        if dispIter
            fprintf('Iteration %d\n',iter)  
            fprintf('--------------------------------\n')
